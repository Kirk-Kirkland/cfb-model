...
def load(season,week):
    ...
    D['qbppa']={s:cfbd('ppa/players/season',year=s,excludeGarbageTime='true') for s in (season-1,season)}
    D['plogs']={}
...
def team_model(D,season,week):
    ...
    P={};PX=CONSTS['PRIOR2026'];CO2,CD2=CONSTS['CO2'],CONSTS['CD2']
    for t in FBS[season]:
        po,pd=END.get(t,END.get('FCS',(-0.15,0.15)));z=(TAL.get(t,m)-m)/sd;r=RET.get(t,0.5)
        px=PX.get(t,{'nh':0,'rd':0.5,'inc':0.13});nh,rd,inc=px['nh'],px['rd'],px['inc']
        P[t]=(float(np.dot(CO2,[po,po*r,z,1,po*nh,nh])),float(np.dot(CD2,[pd,pd*rd,z,inc,pd*nh,nh,1])))
...
# ------------------------------------------------------------------ QB values
def qb_values(D,season,cur_logs,ratings):
    pace={r['team']:r['pace'] for r in ratings}
    games=collections.Counter();seen=set()
    for x in cur_logs:
        if (x['gid'],x['team']) not in seen: seen.add((x['gid'],x['team']));games[x['team']]+=1
    def plays(x):
        a=x['averagePPA']['all'];t=x['totalPPA']['all']
        return abs(t/a) if a not in (0,None) and t is not None else 0
    prev={x['id']:x for x in D['qbppa'][season-1] if x['position']=='QB'}
    cur=[x for x in D['qbppa'][season] if x['position']=='QB']
    q=[x['averagePPA']['all'] for x in D['qbppa'][season-1] if x['position']=='QB' and plays(x)>=100]
    lg=float(np.mean(q)) if q else 0.1;repl=float(np.percentile(q,25)) if q else 0.0
    byteam=collections.defaultdict(list)
    for x in cur: byteam[x['team']].append(x)
    out=[]
    for t,qs in byteam.items():
        qs.sort(key=lambda x:-plays(x));s=qs[0];n=plays(s)
        pv=prev.get(s['id']);pa=pv['averagePPA']['all'] if pv and plays(pv)>=60 else lg
        ps=(n*s['averagePPA']['all']+200*pa)/(n+200)
        if len(qs)>1:
            b_=qs[1];nb=plays(b_);pvb=prev.get(b_['id']);pb=pvb['averagePPA']['all'] if pvb and plays(pvb)>=60 else repl
            pbk=(nb*b_['averagePPA']['all']+200*pb)/(nb+200);bname=b_['name']
        else: pbk=repl;bname='(none listed)';nb=0
        g=max(1,games.get(t,1));share=min(0.8,(n/g)/max(40,pace.get(t,68)))
        val=float(np.clip(0.5*60*(ps-pbk)*share,0,10))  # QB gets ~half the credit for passing-game EPA
        out.append(dict(team=t,starter=s['name'],sp=ps,sn=n,backup=bname,bp=pbk,bn=nb,share=share,val=val))
    return out
# ------------------------------------------------------------------ workbook
...
def read_prev(path):
    ...
    return P
def build(path,season,week,D,games_wk,ratings,G,proj,cur_logs,wx,prev,qbv=()):
    wb=Workbook()
    ...
    ('Market trust (sides)',0.85,'Walk-forward log-loss tuning picked 1.00 (model adds nothing on sides). Sides are NOT bet.'),('Market trust (totals)',0.75,'Walk-forward log-loss tuning 2023-25 picked 0.65-0.80.'),
    ...
    ('Weekly Top 3 budget ($)',100,'Split evenly across the 3 strongest plays (Best Bets, top section).'),('Bets per week (Top N)',3,'Walk-forward: top 3 by edge hit 55-56%. Top 5 drops to 53.7%.'),('Bankroll ($)',500,'Yours.'),('Unit size (% bankroll)',0.02,'BET = 1 unit, LEAN = 0.5.')]
    ...
    HFA,WE,WS,WF,TRS,TRT,SDM,SDT,SBE,SLE,TBE,TLE,TCN,SKIP60,BLOW,OPTA,BIG,WTH,WPEN,PPE,PLE=[I[k[0]] for k in inp[:21]]
    # ---- Injuries
    ...
    hdr(wj,4,['Team (exact name as in Model)','Player','Pos','Status','Pts lost (blank = auto for QB)','Active (Y/N)','Source / note','Pts used'])
    ...
    for i in range(5,60):
        ...
        wj.cell(row=i,column=8,value=f'=IF(A{i}="","",IF(E{i}<>"",E{i},IF(UPPER(C{i})="QB",IFERROR(VLOOKUP(A{i},\'QB Values\'!$A:$H,8,0),0),0)))').number_format='0.0'
    widths(wj,{'A':26,'B':20,'C':6,'D':22,'E':12,'F':10,'G':60,'H':9})
    # ---- Model
    ...
    'Venue','Wind (mph)','Rain %','Temp (F)','Open total','DK total','Bovada total','EPA model total','Blowout adj','Option adj','Weather adj','Total edge','Final proj total','Over prob','Total pick','Total prob','Best total for pick','Total tier','Home ML','Away ML','Home no-vig prob','Model home win prob','ML edge (home)']
    ...
    for i,x in enumerate(gl,2):
        ...
        'Injury adj (home +)':f'=SUMIFS(Injuries!$H:$H,Injuries!$A:$A,{C("Away")},Injuries!$F:$F,"Y")-SUMIFS(Injuries!$H:$H,Injuries!$A:$A,{C("Home")},Injuries!$F:$F,"Y")',
        ...
        'Blowout adj':f'=IF({C("DK home spread")}="",0,{BLOW}*ABS({C("DK home spread")}))',
        'Option adj':f'=IF(OR({C("Home")}="Army",{C("Home")}="Navy",{C("Home")}="Air Force",{C("Away")}="Army",{C("Away")}="Navy",{C("Away")}="Air Force"),{OPTA},0)',
        'Total edge':f'=IF({C("DK total")}="","",{C("EPA model total")}+{C("Blowout adj")}+{C("Option adj")}+{C("Weather adj")}-{C("DK total")})',
        'Final proj total':f'=IF({C("DK total")}="","",{TRT}*{C("DK total")}+(1-{TRT})*({C("EPA model total")}+{C("Blowout adj")}+{C("Option adj")}+{C("Weather adj")}))',
        ...
        'Total tier':f'=IF({C("Total edge")}="","",IF({C("FCS in game")}="Y","No bet: FCS",IF({C("DK total")}>={SKIP60},"Skip: over cap",IF(ABS({C("Total edge")})>={TCN},"BET (check news)",IF(ABS({C("Total edge")})>={TBE},IF(ABS({C("DK home spread")})>={BIG},"LEAN","BET"),IF(ABS({C("Total edge")})>={TLE},"LEAN","Pass"))))))',
        ...
        for n in ('EPA model margin','SP+ margin','FPI margin','Blended margin','Model margin','Edge (pts, + = home)','Final proj margin','EPA model total','Blowout adj','Option adj','Weather adj','Total edge','Final proj total'): wm[C(n)].number_format='0.0;-0.0;0.0'
        ...
        if dk and dk.get('spread') is not None and dk.get('overUnder') is not None and not x['fcs']:
            ...
            te=x['pt']+CONSTS['C_SPREAD']*abs(dk['spread'])+(CONSTS['OPT_ADJ'] if (g['homeTeam'] in CONSTS['OPT_TEAMS'] or g['awayTeam'] in CONSTS['OPT_TEAMS']) else 0)-max(0,wind-12)*0.3-dk['overUnder'];mv=(dk['overUnder']-dk['overUnderOpen']) if dk.get('overUnderOpen') else 0
    ...
    wm.conditional_formatting.add(rg,FormulaRule(formula=[f'${tt}2="BET"'],fill=GOOD));wm.conditional_formatting.add(rg,FormulaRule(formula=[f'${tt}2="LEAN"'],fill=LEANF));wm.conditional_formatting.add(rg,FormulaRule(formula=[f'${tt}2="BET (check news)"'],fill=GOOD));wm.conditional_formatting.add(rg,FormulaRule(formula=[f'LEFT(${tt}2,4)="Skip"'],fill=GREY))
    ...
    for s in sel:
        a=abs(s['te'])
        if a>=10: picks.append(('1',s,'Total',f"{'Over' if s['te']>0 else 'Under'} {s['ou']}",'10+ pt edge: strongest bucket in walk-forward testing (63%). Glance at injury and weather news, then bet.'))
        elif a>=4: picks.append(('1' if abs(s['sp'])<24 else '2',s,'Total',f"{'Over' if s['te']>0 else 'Under'} {s['ou']}",'Totals edge 4+ pts.' + (' Wind %s mph.'%round(s['wind']) if s['wind']>=12 else '')))
    for s in []:
    ...
    for i,(tier,s,typ,bet,note) in enumerate(picks,5):
        ...
        if typ=='Total':
            mt='('+M('EPA model total')+'+'+M('Blowout adj')+'+'+M('Option adj')+'+'+M('Weather adj')+')'
        ...
        for c in range(1,11): wbb.cell(row=r,column=c).fill=fill
    ...
    def rank(p):
        ...
        if typ=='Total' and t in ('1','2'): return (0,-a)
    ...
    # ---- QB values
    wq=wb.create_sheet('QB Values');title(wq,'Auto QB values: points a team loses if the starter sits','0.5 x (starter EPA/play - backup EPA/play) x starter share of plays x 60 (QB gets ~half the credit for pass-game EPA). Heavily shrunk toward last year / replacement level. Sanity check vs news before trusting. Used by Injuries when Pos = QB and Pts is blank.')
    hdr(wq,4,['Team','Starter','Starter EPA/play (shrunk)','Starter plays','Backup','Backup EPA/play (shrunk)','Backup plays','Pts value'],40)
    for i,q in enumerate(sorted(qbv,key=lambda q:-q['val']),5):
        for j,v in enumerate([q['team'],q['starter'],q['sp'],q['sn'],q['backup'],q['bp'],q['bn'],q['val']],1):
            c=wq.cell(row=i,column=j,value=v)
            if j in (3,6): c.number_format='0.000'
            if j in (4,7): c.number_format='0'
            if j==8: c.number_format='0.0'
    widths(wq,{'A':22,'B':22,'E':22});wq.freeze_panes='B5'
    # ---- Key numbers
    ...
    ('Saturday AM','Weather check. Walk-forward shows no real difference between opening and closing lines, so bet whenever the line clears the cutoff.'),
